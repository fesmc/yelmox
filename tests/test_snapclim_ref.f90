program test_snapclim_ref
    ...
    ! tests/test_snapesm_ref.f90 (identical dims/units, proper per-time slices) rather
    ! than snapclim_write_step, whose per-step write of the annual fields is unreliable.
    !
    ! Run from the yelmox root so relative paths (yelmox/, ice_data/, input/, logs/) resolve.
...
contains
    ...
    subroutine dump_init(filename, snp, xc, yc, time0)
    ...
    subroutine dump_step(filename, snp, time, n)
        character(len=*),      intent(IN) :: filename
        type(snapclim_class),  intent(IN) :: snp
        ...
        call nc_write(filename,"mask",  snp%now%mask,  dim1="xc",dim2="yc",dim3="time", &
                        start=[1,1,n],count=[nx,ny,1],ncid=ncid)
        call nc_write(filename,"z_srf", snp%now%z_srf, dim1="xc",dim2="yc",dim3="time", &
                        start=[1,1,n],count=[nx,ny,1],ncid=ncid)
        call nc_write(filename,"tsl", snp%now%tsl, dim1="xc",dim2="yc",dim3="month",dim4="time",&
                        start=[1,1,1,n],count=[nx,ny,12,1],ncid=ncid)
        call nc_write(filename,"tas", snp%now%tas, dim1="xc",dim2="yc",dim3="month",dim4="time",&
                        start=[1,1,1,n],count=[nx,ny,12,1],ncid=ncid)
        call nc_write(filename,"pr",  snp%now%pr,  dim1="xc",dim2="yc",dim3="month",dim4="time",&
                        start=[1,1,1,n],count=[nx,ny,12,1],ncid=ncid)
        call nc_write(filename,"ta_ann", snp%now%ta_ann, dim1="xc",dim2="yc",dim3="time", &
                        start=[1,1,n],count=[nx,ny,1],ncid=ncid)
        call nc_write(filename,"ta_sum", snp%now%ta_sum, dim1="xc",dim2="yc",dim3="time", &
                        start=[1,1,n],count=[nx,ny,1],ncid=ncid)
        call nc_write(filename,"pr_ann", snp%now%pr_ann, dim1="xc",dim2="yc",dim3="time", &
                        start=[1,1,n],count=[nx,ny,1],ncid=ncid)
        call nc_write(filename,"to_ann", snp%now%to_ann, dim1="xc",dim2="yc",dim3="depth",dim4="time",&
                        start=[1,1,1,n],count=[nx,ny,23,1],ncid=ncid)
        call nc_write(filename,"so_ann", snp%now%so_ann, dim1="xc",dim2="yc",dim3="depth",dim4="time",&
                        start=[1,1,1,n],count=[nx,ny,23,1],ncid=ncid)
        call nc_close(ncid)
        return
