...
A summary of commands to get YelmoX running is given below. It assumes you have already cloned yelmox itself and are in the main directory. The example builds with the `ifx` compiler for the `dkrz_levante` machine — substitute your own machine (and compiler) for a different system. Where data paths (`ice_data`, `isostasy_data`) appear, replace `/path/to/...` with the location on your system. This script was generated by running `./install.py -c ifx -m dkrz_levante` on the DKRZ levante cluster.
